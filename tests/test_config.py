...
class HAProxyRenderTests(unittest.TestCase):
    ...
    def test_existing_yaml_enables_runtime_dns_without_new_keys(self) -> None:
        ...
        dns_options = "resolvers whatsapp_dns resolve-prefer ipv4 init-addr last,none"
        self.assertIn(
            f"server g_whatsapp_net_5222 g.whatsapp.net:5222 {dns_options}",
            rendered,
        )
        self.assertIn(
            f"server whatsapp_net_443 whatsapp.net:443 {dns_options}",
            rendered,
        )
    ...
    def test_mixed_hostname_and_literal_ip_only_resolves_hostname(self) -> None:
        ...
        self.assertIn(
            ...
            "resolvers whatsapp_dns resolve-prefer ipv4 init-addr last,none",
            rendered,
        )
        self.assertIn("server whatsapp_net_443 192.0.2.21:443\n", rendered)
